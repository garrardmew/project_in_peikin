...
def trainAndChooseGenes(X, y, gene_name_list=None, gene_num=100, dict_digit_group={}, **kargs):
    try:
    ...
    chooseGenesByWeights(feature_weights, gene_name_list, gene_num, prefix)

# Without training, load previous trained weights and gene_names (shall be in one file). 
def loadPreviousAndChooseGenes(previous_weight_and_gene_pickle, gene_num):
    feature_weights, gene_name_list = loadPickle(previous_weight_and_gene_pickle)
    chooseGenesByWeights(feature_weights, gene_name_list, gene_num, prefix=previous_weight_and_gene_pickle.replace('_feature.weights_genes',''))


def chooseGenesByWeights(feature_weights, gene_name_list, gene_num, prefix, do_abs=True): 
    if do_abs:
...
#     parser.add_argument('-n', '--gene_num', dest="gene_num", default=100, help="Gene numbers to choose", nargs=1, action="store", type=int)
    ...
    additional_arg_list = [['-N', '--gene_num', 'gene_num', 100, 'Gene numbers to choose (For binary-class method, genes generated by different classification instances will be joined)', int], 
                           ...
                           ['-E', '--for_each_class', 'for_each_class', "False", 'Path to a previous .weights_genes file. ', str], 
                           ]
    ...
    if l1(args.previous_weighted_gene) is None:
        ...
        loadPreviousAndChooseGenes(l1(args.previous_weighted_gene), gene_num=l1(args.gene_num))    
